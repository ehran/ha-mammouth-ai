...
class MammouthDataUpdateCoordinator(DataUpdateCoordinator[Dict[str, Any]]):
    ...
    async def async_chat_completion(
        ...
        **kwargs: Any,
        ...
        """Get a chat completion from Mammouth AI.

        Returns the full "message" object from the API response (dict with
        at least "role" and "content", and possibly "tool_calls") rather than
        just the text, so the caller can implement a tool-calling loop.
        """
